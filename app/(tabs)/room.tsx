...
import { collection, addDoc, deleteDoc, doc, getDocs } from "firebase/firestore"; // Add getDocs
...
const Room = () => {
    ...
    const [rooms, setRooms] = useState<
        { id: string; name: string; section: string; startTime: string; endTime: string; code: string }[]
    >([]);
    ...
    const generateRoomCode = () => Math.floor(1000000 + Math.random() * 9000000).toString();

    const toggleStartAmPm = () => setStartAmPm((prev) => (prev === "AM" ? "PM" : "AM"));
    const toggleEndAmPm = () => setEndAmPm((prev) => (prev === "AM" ? "PM" : "AM"));

    const validateHourInput = (value: string): string => {
        const numericValue = value.replace(/[^0-9]/g, ""); // Remove non-numeric characters
        if (numericValue === "") return ""; // Allow empty input
        const parsedValue = Math.min(Math.max(parseInt(numericValue, 10), 1), 12); // Restrict within 1-12
    ...
    const handleCreateRoom = () => {
        if (roomName && roomSection && startHour && startMinute && endHour && endMinute) {
            const newRoom = {
                name: roomName,
                section: roomSection,
    ...
    const confirmRoomCreation = async () => {
        try {
            if (currentRoom) {
                ...
                const docRef = await addDoc(collection(FIREBASE_DB, "rooms"), currentRoom);
                console.log("Room added with ID:", docRef.id);

                // Add room to local state
                setRooms((prevRooms) => [...prevRooms, { ...currentRoom, id: docRef.id }]);
            }
            ...
            {rooms.map((room) => (
                <View key={room.id} style={styles.roomCard}>
                    ...
                    <View style={styles.roomCodeContainer}>
                        <Text style={styles.roomCodeText}>Code: {room.code}</Text>
                        <TouchableOpacity onPress={() => handleCopyToClipboard(room.code)}>
                ...
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        ...
                        <View style={styles.timeInputContainer}>
                            <View style={styles.timeGroup}>
                                <TextInput
                                    ...
                                    value={startHour}
                                    onChangeText={(value) => setStartHour(validateHourInput(value))}
                                ...
                                <TextInput
                                    ...
                                    style={styles.timeInput}
                                    value={startMinute}
                                    onChangeText={(value) => setStartMinute(validateMinuteInput(value))}
                                    keyboardType="numeric"
                                    maxLength={2}
                                />
                                <TouchableOpacity style={[styles.button, styles.amPmButton]} onPress={toggleStartAmPm}>
                                    <Text style={styles.buttonText}>{startAmPm}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View style={styles.timeInputContainer}>
                            <View style={styles.timeGroup}>
                                <TextInput
                                    ...
                                    style={styles.timeInput}
                                    value={endHour}
                                    onChangeText={(value) => setEndHour(validateHourInput(value))}
                                    keyboardType="numeric"
                                    maxLength={2}
                                />
                                <Text style={styles.colon}>:</Text>
                                <TextInput
                                    ...
                                    style={styles.timeInput}
                                    value={endMinute}
                                    onChangeText={(value) => setEndMinute(validateMinuteInput(value))}
                                    keyboardType="numeric"
                                    maxLength={2}
                                />
                                <TouchableOpacity style={[styles.button, styles.amPmButton]} onPress={toggleEndAmPm}>
                                    <Text style={styles.buttonText}>{endAmPm}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                        <View style={styles.buttonContainer}>
                            ...
                            <TouchableOpacity style={[styles.button, styles.confirmButton]} onPress={handleCreateRoom}>
            ...
            >
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        ...
                        {currentRoom && (
                            <View>
                                ...
                                <View style={styles.roomCodeRow}>
                                    <Text style={styles.roomCodeText}>Room Code: {currentRoom.code}</Text>
                                    <TouchableOpacity onPress={() => handleCopyToClipboard(currentRoom.code)}>
                        ...
                        <View style={styles.buttonContainer}>
                            <TouchableOpacity style={[styles.button, styles.cancelButton]} onPress={cancelRoomCreation}>
                                <Text style={styles.buttonText}>CANCEL</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={[styles.button, styles.confirmButton]} onPress={confirmRoomCreation}>
