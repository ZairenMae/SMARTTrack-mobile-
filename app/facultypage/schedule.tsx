...
<ScrollView style={styles.eventsContainer} contentContainerStyle={styles.eventsContentContainer}>
  ...
  <FontAwesome5 name="minus-square" size={24} color="#800000" />
    ...
    <TextInput
      ...
      onChangeText={(text) => setCurrentEvent({ ...currentEvent, students: text })}
      style={styles.input}
    />
    ...
    <View style={styles.toggleContainer}>
      <TouchableOpacity
        onPress={() => setCurrentEvent({ ...currentEvent, notify: !currentEvent.notify })}
        style={styles.toggleButton}
      >
        <FontAwesome5
          name={currentEvent.notify ? "check-square" : "square"}
          size={24}
          color="#800000"
        />
      ...
      <TouchableOpacity
        onPress={() => setCurrentEvent({ ...currentEvent, wholeDay: !currentEvent.wholeDay })}
        style={styles.toggleButton}
      >
        <FontAwesome5
          name={currentEvent.wholeDay ? "check-square" : "square"}
          size={24}
          color="#800000"
        />
  ...
  <View style={styles.timeRow}>
    <TextInput
      placeholder="HH"
      value={currentEvent.startTime?.split(':')[0] || ''}
      onChangeText={(text) => {
        const [hours, minutes] = (currentEvent.startTime || '00:00 AM').split(/[: ]/);
        setCurrentEvent({
          ...currentEvent,
          startTime: `${text.padStart(2, '0')}:${minutes || '00'} ${currentEvent.startTime?.split(' ')[1] || 'AM'}`,
        });
      }}
    ...
    <TextInput
      placeholder="MM"
      value={currentEvent.startTime?.split(':')[1]?.split(' ')[0] || ''}
      onChangeText={(text) => {
        const [hours, minutes] = (currentEvent.startTime || '00:00 AM').split(/[: ]/);
        setCurrentEvent({
          ...currentEvent,
          startTime: `${hours || '00'}:${text.padStart(2, '0')} ${currentEvent.startTime?.split(' ')[1] || 'AM'}`,
        });
      }}
      style={styles.timeInput}
      keyboardType="numeric"
    ...
    />
    <TouchableOpacity
      style={[styles.amPmToggle, currentEvent.startTime?.includes('AM') ? styles.activeToggle : null]}
      onPress={() => {
        const [hours, minutes] = (currentEvent.startTime || '00:00 AM').split(/[: ]/);
        setCurrentEvent({
          ...currentEvent,
          startTime: `${hours || '00'}:${minutes || '00'} AM`,
        });
      }}
    >
    ...
    <TouchableOpacity
      style={[styles.amPmToggle, currentEvent.startTime?.includes('PM') ? styles.activeToggle : null]}
      onPress={() => {
        const [hours, minutes] = (currentEvent.startTime || '00:00 AM').split(/[: ]/);
        setCurrentEvent({
          ...currentEvent,
          startTime: `${hours || '00'}:${minutes || '00'} PM`,
        });
      }}
    >
  ...
  <View style={styles.timeRow}>
    <TextInput
      placeholder="HH"
      value={currentEvent.endTime?.split(':')[0] || ''}
      onChangeText={(text) => {
        const [hours, minutes] = (currentEvent.endTime || '00:00 AM').split(/[: ]/);
        setCurrentEvent({
          ...currentEvent,
          endTime: `${text.padStart(2, '0')}:${minutes || '00'} ${currentEvent.endTime?.split(' ')[1] || 'AM'}`,
        });
      }}
      style={styles.timeInput}
      keyboardType="numeric"
    ...
    <TextInput
      placeholder="MM"
      value={currentEvent.endTime?.split(':')[1]?.split(' ')[0] || ''}
      onChangeText={(text) => {
        const [hours, minutes] = (currentEvent.endTime || '00:00 AM').split(/[: ]/);
        setCurrentEvent({
          ...currentEvent,
          endTime: `${hours || '00'}:${text.padStart(2, '0')} ${currentEvent.endTime?.split(' ')[1] || 'AM'}`,
        });
      }}
      style={styles.timeInput}
      keyboardType="numeric"
    ...
    />
    <TouchableOpacity
      style={[styles.amPmToggle, currentEvent.endTime?.includes('AM') ? styles.activeToggle : null]}
      onPress={() => {
        const [hours, minutes] = (currentEvent.endTime || '00:00 AM').split(/[: ]/);
        setCurrentEvent({
          ...currentEvent,
          endTime: `${hours || '00'}:${minutes || '00'} AM`,
        });
      }}
    >
      <Text style={styles.amPmText}>AM</Text>
    ...
    <TouchableOpacity
      style={[styles.amPmToggle, currentEvent.endTime?.includes('PM') ? styles.activeToggle : null]}
      onPress={() => {
        const [hours, minutes] = (currentEvent.endTime || '00:00 AM').split(/[: ]/);
        setCurrentEvent({
          ...currentEvent,
          endTime: `${hours || '00'}:${minutes || '00'} PM`,
        });
      }}
    >
      <Text style={styles.amPmText}>PM</Text>
    ...
    <View style={styles.buttonContainer}>
      ...
      >
        <Text style={styles.createButtonText}>{isEditing ? "Update" : "Create"}</Text>
      </TouchableOpacity>
